...
func (t *MqttTrigger) Stop() error {
	...
	config := readConfig("mqtt.json") // config data
	log.Debug("Unsubscribing from topic: ", config.BrokerTopic)
	if token := t.client.Unsubscribe(config.BrokerTopic); token.Wait() && token.Error() != nil {
		log.Errorf("Error unsubscribing from topic %s: %s", config.BrokerTopic, token.Error())
	}
